...
REPLACE_DIGITS = {
    'ноль': '0', 'один': '1', 'два': '2', 'три': '3', 'четыре': '4', 'пять': '5', 'шесть': '6', 'восемь': '8',
    'семь': '7', 'девять': '9', 'десять': '10',
    ...
    'полторы': '1.5*',
    'нуль': '0',
    'нулю': '0', 'одному': '1', 'двум': '2', 'трем': '3', 'четырем': '4', 'трём': '3', 'четырём': '4', 'пяти': '5',
    'шести': '6', 'восьми': '8', 'семи': '7', 'девяти': '9', 'десяти': '10',
...
ERRORS = {
    ...
    4: ['В уравнении непарные скобки. Скажите помощь скобки',
        'Число отрывающихся скобок не равно числу закрывающихся. Скажите помощь скобки'],
    5: ['Уравнение должно содержать переменную x,y или z.', 'Похоже в вашем уравнении нет неизвестной',
        'Я могу решать уравнения с x,y или z. Попробуйте перефразировать'],
...
HELP_TEXTS = {
    ...
    'вычисли': ['2^5*sqrt(16)',
                'Я могу вычислять числовое значение выражения без переменных, для этого скажите Вычисли и продиктуйте выражение.'],
    'упрости': ['(2x-3y)(3y-2x)-12xy',
                'Я могу упрощать алгебраические выражения, для этого скажите Упрости и продиктуйте выражение.'],
    'разложи': ['(x+y)^3',
                'Я могу раскладывать на множители алгебраические выражения, для этого скажите Разложи и продиктуйте выражение.'],
    'предел': ['1/(x+1)',
               'Я могу найти предел по х, при х->0, для скажи Предел и продиктуйте выражение'],
    'интеграл': ['x*cos(x)',
                 'Я могу найти интеграл по х, для этого скажи Интеграл и продиктуйте выражение. Тригонометрические пишутся с аргументом в скобках'],
    'производная': ['1/x+x',
                    'Я могу найти первую производную по x, для этого скажи Производная и продиктуйте выражение'],
}
...
COMMAND_FACT = ['разложи', 'разложить', 'разложение', 'представить']
#Запросы для расчета проиводной
COMMAND_PROIZVODNAYA=['производная', 'производную', 'проиводные']
# Запросы для расчета интеграла неопределеного
COMMAND_INTEGRAL=['интеграл', 'неопределенный', 'интегралы', 'неопределенные', 'нтеграл']
# Запросы для решения системы уравнений
COMMAND_SISTEMA=['система', 'системы', 'слау', 'систему']
# Запрос на нахождения предела при n->00
COMMAND_PREDEL=['предел', 'пределы' ]
# Запрос на нахождения решения неравенства
COMMAND_NERAVENSTVO=['неравенство', 'неравесту', 'неравенствы']
# ответ на некорректный запрос
DEFAULT_ANSWER = ['У меня нет ответа.', 'Я просто работаю с выражениями.', 'Этого я не понимаю.', 'Я не по этой части.',
                  'Я понимаю определенные запросы.']
...
class Processing:
    ...
    def process(self):
        ...
        if self.task == 'solve':
            ...
            self._factorize()
        elif self.task == 'predel':
            self.predel()
        elif self.task == 'integral':
            self.integral()
        elif self.task == 'neravenstvo':
            self.neravenstvo()
        elif self.task == 'proizvodnaya':
            self.proizvodnaya()
...
# Предварительная подготовка выражения
    ...
    def _prepare(self):
        ...
        if any(c in self.equation for c in COMMAND_FACT):
            self.task = 'factorize'
        if any(c in self.equation for c in COMMAND_INTEGRAL):
            self.task = 'integral'
        if any(c in self.equation for c in COMMAND_PREDEL):
            self.task = 'predel'
        if any(c in self.equation for c in COMMAND_PROIZVODNAYA):
            self.task = 'proizvodnaya'
        if any(c in self.equation for c in COMMAND_NERAVENSTVO):
            self.task = 'neravenstvo'
    ...
    def _factorize(self):
        ...
        if self.check_errors():
            return
        try:
            self.answer = expand(self.equation)
        except Exception:
            self.answer = 'Ошибка в выражении: ' + self.equation

    def integral(self):
        x = symbols('x')
        if 'y' in self.equation or 'z' in self.equation:
            self.answer = 'Пока интегрировать можно только по x'
            return
        # Проверка на ошибки
        if self.check_errors():
            return
        try:
            self.answer = integrate(self.equation, x)
        except Exception:
            self.answer = 'Ошибка в выражении: ' + self.equation

    def predel(self):
        if 'y' in self.equation or 'z' in self.equation:
            self.answer = 'Пока найти предел можно только по x'
            return
        # Проверка на ошибки
        if self.check_errors():
            return
        try:
            ...
            self.answer = self.equation.limit('x',0)
        except Exception :
            self.answer = 'Ошибка в выражении: ' + self.equation

    def neravenstvo(self):
        return

    def proizvodnaya(self):
        x = symbols('x')
        if 'y' in self.equation or 'z' in self.equation:
            self.answer = 'Пока найти первую производую можно только по x'
            return
        # Проверка на ошибки
        if self.check_errors():
            return
        try:
            f = sympify(self.equation)
            self.answer = f.diff(x)
        except NotImplementedError :
            self.answer = 'Ошибка в выражении: ' + self.equation
...
def handle_dialog(req, res, user_storage):
    ...
    user_command = req.command.lower().replace('спроси Решала ответит', '').strip()
    # исходное выражение выбросим лишние слова
    ...
    user_message = req.original_utterance.lower().strip()
    ...
    if not process.first_word or req.is_new_session:
        user_answer = 'Привет!\nЯ помогаю решать уравнения и примеры.\n' + \
    ...
    if user_message == 'почему':
        res.set_text(' Я вычисляю результат, а не рассказываю как решать. \nЭто будет в следующей версии...')
        res.set_buttons(user_storage['suggests'])
        return res, user_storage
    # ответ нет
    if user_message == 'нет':
    ...
    if 'как' in user_tokens and 'зовут' in user_tokens:
        res.set_text('Меня зовут Решала, версии 1.0. ')
        res.set_buttons(user_storage['suggests'])
        return res, user_storage
    ...
    # если похвалили
    if 'спасибо' in user_tokens or user_message in [
        'правильно',
        'верно',
        'хорошо',
        'молодец',
    ...
    if any(i in user_tokens for i in [
        ...
        if len(s) == 1:
            user_answer = 'Я умею решать уравнения с одной неизвестной x,y или z, упрощать и вычислять алгебраические выражения.\n' + \
                          ...
                          'В тригонометрических функциях аргумент указывается в скобках \n'+ \
            'По всем пожеланиям можете обращться на почту'
    ...
    user_answer = str(
        random.choice(DEFAULT_ANSWER) + ' ' + DEFAULT_ENDING if process.answer is False else process.answer)
    res.set_text(user_answer)
    # озвучка результата
